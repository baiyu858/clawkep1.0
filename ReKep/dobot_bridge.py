...
DEFAULT_XTRAINER_SDK_DIR = REPO_DIR.parent / ".downloads" / "third_party" / "dobot_xtrainer"
...
DEFAULT_LOCAL_DOBOT_HOST = "127.0.0.1"
...
DEFAULT_XTRAINER_ZMQ_TIMEOUT_ENV = "REKEP_XTRAINER_ZMQ_TIMEOUT_MS"
DEFAULT_XTRAINER_ZMQ_TIMEOUT_MS = 30000
...
def resolve_camera_source(args):
    ...
    if fallback:
        return fallback
    driver = resolve_dobot_driver(args)
    host = resolve_dobot_host(args, driver=driver)
    if driver == "xtrainer_zmq" and host:
        return f"realsense_zmq://{host}:{DEFAULT_REMOTE_REALSENSE_ZMQ_PORT}/{DEFAULT_REALSENSE_ZMQ_TOPIC}"
    return "0"


def resolve_dobot_driver(args):
    explicit = read_string(getattr(args, "dobot_driver", None)).lower()
    if explicit:
        return explicit
    fallback = os.environ.get(DEFAULT_DOBOT_DRIVER_ENV, "xtrainer_zmq").strip().lower()
    return fallback or "xtrainer_zmq"


def resolve_dobot_host(args, *, driver: str | None = None):
    ...
    if from_env:
        return from_env
    selected_driver = read_string(driver, resolve_dobot_driver(args)).lower()
    if selected_driver == "xtrainer_zmq":
        return DEFAULT_REMOTE_DOBOT_HOST
    return DEFAULT_LOCAL_DOBOT_HOST


def resolve_dobot_port(args, *, driver: str | None = None):
...
def resolve_dobot_move_port(args, *, driver: str | None = None):
...
def resolve_runtime_hardware_profile(args):
    driver = resolve_dobot_driver(args)
    host = resolve_dobot_host(args, driver=driver)
    port = resolve_dobot_port(args, driver=driver)
    move_port = resolve_dobot_move_port(args, driver=driver)
...
class XtrainerZmqDobotAdapter(RobotAdapter):
    def __init__(self, host, port, request_timeout_ms=None):
        self.host = str(host)
        self.port = int(port)
        resolved_timeout = request_timeout_ms
        if resolved_timeout is None:
            resolved_timeout = os.environ.get(DEFAULT_XTRAINER_ZMQ_TIMEOUT_ENV, DEFAULT_XTRAINER_ZMQ_TIMEOUT_MS)
        try:
            resolved_timeout = int(float(resolved_timeout))
        except Exception:
            resolved_timeout = int(DEFAULT_XTRAINER_ZMQ_TIMEOUT_MS)
        self.request_timeout_ms = max(1000, int(resolved_timeout))
        self._context = None
        self._socket = None
        self._pickle = None
        self._np = None
    ...
    def _probe_tcp_endpoint(self, timeout_s=0.8):
        try:
            with socket.create_connection((self.host, self.port), timeout=max(0.1, float(timeout_s))):
                return True, ""
        except Exception as exc:
    ...
    def _is_timeout_error(exc):
        ...
        return any(sig in text for sig in timeout_signatures)
    ...
    def _is_connectivity_error(exc):
        ...
        return any(sig in text for sig in connectivity_signatures)

    def _format_rpc_exception(self, method, exc):
        detail = str(exc).strip()
        endpoint = f"{self.host}:{self.port}"
        if self._is_timeout_error(exc):
            probe_timeout_s = min(1.5, max(0.3, float(self.request_timeout_ms) / 1000.0 * 0.10))
            probe_ok, probe_error = self._probe_tcp_endpoint(timeout_s=probe_timeout_s)
            if not probe_ok:
                return (
                    f"xtrainer_zmq rpc timeout ({method}): endpoint {endpoint} unreachable "
                    f"({probe_error}). robot service is likely not listening / crashed."
                )
            return (
                f"xtrainer_zmq rpc timeout ({method}): endpoint {endpoint} reachable but no reply within "
                f"{int(self.request_timeout_ms)} ms. robot controller may be alarmed / blocked."
            )
        if self._is_connectivity_error(exc):
            return (
                f"xtrainer_zmq rpc connectivity failed ({method}): {detail}. "
                f"endpoint={endpoint}"
            )
        return f"xtrainer_zmq rpc failed ({method}): {detail}"
    ...
    def _rpc(self, method, args=None):
        ...
        try:
            ...
            payload = self._pickle.loads(response)
            if isinstance(payload, dict) and "error" in payload:
                raise RuntimeError(f"xtrainer_zmq remote error ({method}): {payload.get('error')}")
            return payload
        except Exception as exc:
            try:
                self._reset_socket()
                ...
                pass
            raise RuntimeError(self._format_rpc_exception(method, exc)) from exc
...
def run_preflight(args, require_vlm=False):
    ...
    action_name = read_string(getattr(args, "action", ""), "").lower()
    execution_mode = resolve_rekep_execution_mode(args)
    ...
    if driver == "xtrainer_zmq":
        ...
        timeout_raw = os.environ.get(DEFAULT_XTRAINER_ZMQ_TIMEOUT_ENV, str(DEFAULT_XTRAINER_ZMQ_TIMEOUT_MS))
        try:
            timeout_ms = max(1000, int(float(timeout_raw)))
        except Exception:
            timeout_ms = int(DEFAULT_XTRAINER_ZMQ_TIMEOUT_MS)
        notes.append(f"xtrainer_zmq rpc timeout_ms: {timeout_ms}")
    ...
    requires_prompt_template = (
        action_name in {"execute", "execute_background", "execute_worker", "longrun_start", "longrun_worker"}
        and execution_mode in SUPPORTED_REKEP_EXECUTION_MODES
    )
    if requires_prompt_template:
        prompt_template_path = REPO_DIR / "vlm_query" / "prompt_template.txt"
        if not prompt_template_path.exists():
            blockers.append("rekep-prompt-template-missing")
            notes.append(f"missing prompt template: {prompt_template_path}")
...
def build_execute_worker_argv(args, job_id, state_dir):
    ...
    argv = [
        ...
        "--dobot_port",
        str(hardware_profile.robot_port),
        "--dobot_move_port",
        str(hardware_profile.robot_move_port),
        "--xtrainer_sdk_dir",
