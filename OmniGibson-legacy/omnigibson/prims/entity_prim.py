...
class EntityPrim(XFormPrim):
    ...
    def _articulation_view(self):
        ...
        if og.sim.is_playing() and self.initialized:
            ...
            physics_view = getattr(self._articulation_view_direct, "_physics_view", None)
            assert self._articulation_view_direct.is_physics_handle_valid() and \
                physics_view is not None and physics_view.check(), \
    ...
    def get_coriolis_and_centrifugal_forces(self, clone=True):
        ...
        forces = self._articulation_view.get_coriolis_and_centrifugal_forces(clone=clone)
        forces = np.array(forces).reshape(-1)
        if forces.shape[0] == self.n_dof + 6:
            forces = forces[6:]
        return forces.reshape(self.n_dof)
    ...
    def get_generalized_gravity_forces(self, clone=True):
        ...
        forces = self._articulation_view.get_generalized_gravity_forces(clone=clone)
        forces = np.array(forces).reshape(-1)
        if forces.shape[0] == self.n_dof + 6:
            forces = forces[6:]
        return forces.reshape(self.n_dof)
    ...
    def get_mass_matrix(self, clone=True):
        ...
        mass_matrix = np.array(self._articulation_view.get_mass_matrices(clone=clone))
        if mass_matrix.ndim == 3:
            mass_matrix = mass_matrix.squeeze(axis=0)
        if mass_matrix.ndim == 1:
            dim = int(np.sqrt(mass_matrix.shape[0]))
            mass_matrix = mass_matrix.reshape(dim, dim)
        if mass_matrix.shape == (self.n_dof + 6, self.n_dof + 6):
