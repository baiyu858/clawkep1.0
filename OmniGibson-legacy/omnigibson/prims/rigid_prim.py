...
class RigidPrim(XFormPrim):
    ...
    def update_handles(self):
        ...
        if not self.kinematic_only:
            try:
                ...
                log.warning(
                    f"Failed to initialize rigid prim view for {self.prim_path}; "
                    f"falling back to USD pose access. Error: {exc}"
                )
    ...
    def set_position_orientation(self, position=None, orientation=None):
        ...
        try:
            ...
            if position is not None:
                position = position[0]
            if orientation is not None:
                orientation = orientation[0][[1, 2, 3, 0]]
    ...
    def set_local_pose(self, position=None, orientation=None):
        ...
        try:
            ...
            if orientation is not None:
                orientation = orientation[0][[1, 2, 3, 0]]
            super().set_local_pose(position=position, orientation=orientation)
        PoseAPI.invalidate()
    ...
    def get_local_pose(self):
        ...
        try:
            ...
            positions, orientations = super().get_local_pose()
        if self.kinematic_only:
    ...
    def _rigid_prim_view(self):
        ...
        if not self.kinematic_only and og.sim.is_playing() and self.initialized:
            try:
                handle_valid = self._rigid_prim_view_direct.is_physics_handle_valid()
                backend_valid = handle_valid and self._rigid_prim_view_direct._physics_view.check()
            except Exception:
                backend_valid = False
