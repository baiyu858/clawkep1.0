...
def _resolve_experience_file():
    ...
    isaac_root = Path(os.environ.get("ISAAC_PATH", exp_path.parent))
    version_file = isaac_root / "VERSION"
    isaac_version = version_file.read_text().strip() if version_file.exists() else ""

    if isaac_version.startswith("2023.1.1"):
        kit_file = Path(__file__).parent / "omnigibson.kit"
        kit_file_target = exp_path / "omnigibson.kit"
        try:
        ...
        return kit_file_target

    for candidate in ("isaacsim.exp.base.python.kit", "isaacsim.exp.full.kit"):
        candidate_path = exp_path / candidate
        if candidate_path.exists():
            log.warning(
                "Using %s with Isaac Sim %s instead of legacy omnigibson.kit for compatibility.",
                candidate,
                isaac_version or "unknown",
            )
            return candidate_path

    raise FileNotFoundError("Could not resolve a compatible Isaac Sim experience file.")
...
def _launch_app():
    ...
    experience_path = _resolve_experience_file()

    launch_context = nullcontext if gm.DEBUG else suppress_omni_log
    with launch_context(None):
        app = lazy.omni.isaac.kit.SimulationApp(config_kwargs, experience=str(experience_path.resolve(strict=True)))
    ...
    isaac_version = lazy.omni.isaac.version.get_version()[0]
    if not isaac_version.startswith(("4.", "5.")):
