...
try:
    ...
    class OmniUtils:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError("omni.particle.system.core is not available in this Isaac Sim build")

    class OmniCore:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError("omni.particle.system.core is not available in this Isaac Sim build")
...
try:
    from isaacsim.core.prims.impl.xform_prim import XFormPrim as _IsaacXFormPrim
    from isaacsim.core.simulation_manager import SimulationManager as _IsaacSimulationManager
except ModuleNotFoundError:
    _IsaacXFormPrim = None
    _IsaacSimulationManager = None

DEG2RAD = math.pi / 180.0


def _is_physics_view_usable(physics_view):
    if physics_view is None:
        return False

    check = getattr(physics_view, "check", None)
    if callable(check):
        try:
            if not check():
                return False
    ...
    for attr in ("count", "max_shapes", "max_dofs"):
        try:
            getattr(physics_view, attr)
            return True
        except Exception:
            continue
...
def _safe_is_physics_handle_valid(self):
    if _IsaacSimulationManager is None:
        return getattr(self, "_physics_view", None) is not None

    if _IsaacSimulationManager.get_physics_sim_view() is None:
        return False

    return _is_physics_view_usable(getattr(self, "_physics_view", None))


def _safe_rigid_on_physics_ready(self, event):
    if _IsaacXFormPrim is None or _IsaacSimulationManager is None:
        return

    _IsaacXFormPrim._on_physics_ready(self, event)
    simulation_view = _IsaacSimulationManager.get_physics_sim_view()
    ...
    try:
        physics_view = simulation_view.create_rigid_body_view(
            [regular_expression.replace(".*", "*") for regular_expression in self._regex_prim_paths]
        )
        if not _is_physics_view_usable(physics_view):
        ...
        carb.log_warn(
            f"Skipping rigid body tensor view for {getattr(self, '_regex_prim_paths', None)}: {exc}"
        )
        self._physics_view = None
        self._num_shapes = 0
        return
    ...
    if self._track_contact_forces:
        try:
            ...
            carb.log_warn(f"Failed to initialize rigid contact view for {self._regex_prim_paths}: {exc}")


_safe_is_physics_handle_valid.__name__ = "is_physics_handle_valid"
_safe_rigid_on_physics_ready.__name__ = "_on_physics_ready"
_ArticulationView.is_physics_handle_valid = _safe_is_physics_handle_valid
_RigidPrimView.is_physics_handle_valid = _safe_is_physics_handle_valid
_RigidPrimView._on_physics_ready = _safe_rigid_on_physics_ready
