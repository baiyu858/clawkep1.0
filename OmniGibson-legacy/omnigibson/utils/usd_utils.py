...
class PoseAPI:
    ...
    def get_world_pose_with_scale(cls, prim_path):
        ...
        """
        cls._refresh()
        getter = getattr(lazy.omni.isaac.core.utils.xforms, "_get_world_pose_transform_w_scale", None)
        if getter is None:
            getter = getattr(lazy.isaacsim.core.utils.xforms, "_get_world_pose_transform_w_scale", None)
        if getter is None:
            raise AttributeError("Isaac Sim xforms module does not expose _get_world_pose_transform_w_scale")
        return np.array(getter(prim_path)).T
