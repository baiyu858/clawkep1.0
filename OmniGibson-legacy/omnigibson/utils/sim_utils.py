...
def meets_minimum_isaac_version(minimum_version):
    test_version = lazy.omni.isaac.version.get_version()[0]

    def _parse_numeric_triplet(version):
        parts = []
    ...
    test_nums = _parse_numeric_triplet(test_version)
    minimum_nums = _parse_numeric_triplet(minimum_version)
    if test_nums is None or minimum_nums is None:
        return False

    # Isaac Sim 4.x / 5.x moved away from the 2023.x.y naming scheme.
    # Treat those major versions as newer than the 2023.1.1 baseline required by legacy OmniGibson.
    test_major = test_nums[0]
    minimum_major = minimum_nums[0]
    if test_major < 1000 <= minimum_major:
        return test_major >= 4
    if minimum_major < 1000 <= test_major:
        return True

    return python_utils.meets_minimum_version(
        ".".join(str(num) for num in test_nums),
        ".".join(str(num) for num in minimum_nums),
    )
