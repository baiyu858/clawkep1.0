...
function createProps(overrides: Partial<ChatProps> = {}): ChatProps {
  return {
    ...
    onToggleRekepLogPanel: () => undefined,
    ...overrides,
  };
}
...
describe("chat view", () => {
  ...
  it("renders a dedicated ReKep log panel for rekep_sim output", () => {
    const container = document.createElement("div");
    render(
      renderChat(
        createProps({
    ...
    const panel = container.querySelector(".rekep-log-panel");
    expect(panel).not.toBeNull();
    expect(panel?.textContent).toContain("ReKep Live Log · pen");
    expect(panel?.textContent).toContain("stage 1 ready");
    expect(panel?.textContent).toContain("/tmp/rekep.log");
  });

  it("renders the ReKep log panel collapsed and toggles it", () => {
    const container = document.createElement("div");
    const onToggleRekepLogPanel = vi.fn();
    render(
      renderChat(
        createProps({
          rekepLogPanelOpen: false,
          onToggleRekepLogPanel,
          toolMessages: [
            {
              role: "assistant",
              toolCallId: "tool-2",
    ...
    expect(container.querySelector(".rekep-log-panel__body")).toBeNull();
    ...
    expect(onToggleRekepLogPanel).toHaveBeenCalledTimes(1);
  });
});
