...
describe("handleChatEvent", () => {
  ...
  it("processes aborted from own run without message and empty stream", () => {
    ...
    expect(state.chatMessages[1]).toMatchObject({
      role: "assistant",
      content: [{ type: "text", text: "Run aborted before a final reply was produced." }],
    });
  });

  it("appends a visible assistant error message for error events", () => {
    const state = createState({
      sessionKey: "main",
      chatRunId: "run-1",
      chatStream: "Working...",
    ...
    expect(state.chatStream).toBe(null);
    expect(state.chatStreamStartedAt).toBe(null);
    expect(state.lastError).toBe("tool failed");
    expect(state.chatMessages).toHaveLength(1);
...
describe("loadChatHistory", () => {
  ...
  it("preserves local status messages when history does not contain them", async () => {
    const localStatus = {
      role: "assistant",
      content: [{ type: "text", text: "Error: tool failed" }],
      __openclaw: { kind: "local-status", state: "failed", runId: "run-1" },
    };
    const mockClient = {
      request: vi.fn().mockResolvedValue({
        messages: [{ role: "user", content: [{ type: "text", text: "Run task" }] }],
      }),
    };
    const state = createState({
      client: mockClient as unknown as ChatState["client"],
      connected: true,
      chatMessages: [localStatus],
    });
  ...
  it("drops local user placeholder once matching history arrives", async () => {
    const localUser = {
      role: "user",
      content: [{ type: "text", text: "Run task" }],
      __openclaw: { kind: "local-user", state: "pending", runId: "run-1" },
    };
    const persistedUser = { role: "user", content: [{ type: "text", text: "Run task" }] };
    const mockClient = {
      request: vi.fn().mockResolvedValue({
