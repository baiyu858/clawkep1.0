  ...
  const client = new GatewayBrowserClient({
    ...
    onClose: ({ code, reason, error }) => {
      ...
      if (
        code !== 1012 &&
        host.lastError &&
        (host.chatRunId || host.chatSending || (host.toolStreamOrder?.length ?? 0) > 0) &&
        Array.isArray(host.chatMessages)
      ) {
        host.chatMessages = [
          ...host.chatMessages,
          {
            ...
            content: [
              {
                type: "text",
                text: `Connection lost while waiting for the result: ${host.lastError}`,
              },
            ],
            timestamp: Date.now(),
            __openclaw: {
              kind: "local-status",
              state: "disconnected",
              runId: host.chatRunId ?? undefined,
            },
          },
        ];
      }
...
function handleTerminalChatEvent(
  ...
  if (hadToolEvents && (state === "final" || state === "error" || state === "aborted")) {
