...
vi.hoisted(() => {
  ...
  vi.stubGlobal("window", {
    location: { href: "http://localhost:8787/openclaw/" },
  });
});
...
describe("connectGateway", () => {
  ...
  it("appends a chat-visible disconnect message when an active run loses connection", () => {
    const host = createHost();
    ...
    expect(host.chatMessages[0]).toMatchObject({
      ...
      content: [
        {
          type: "text",
          text: "Connection lost while waiting for the result: disconnected (1006): no reason",
        },
      ],
    });
  });
