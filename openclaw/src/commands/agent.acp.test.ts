...
function mockAcpManager(params: {
  ...
  getAcpSessionManagerSpy.mockReturnValue({
    ...
    initializeSession:
      ...
      (async () => {
        throw new Error("initializeSession should not be called in this test");
      }),
    resolveSession:
...
describe("agentCommand ACP runtime routing", () => {
  ...
  it("auto-initializes runtime-backed main sessions before the first ACP turn", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
      ...
      await agentCommand({ message: "ping", sessionKey: "agent:main:main" }, runtime);
