...
type AgentRuntimeBackedAcpConfig = {
...
function resolveAgentRuntimeBackedAcpConfig(params: {
  cfg: ReturnType<typeof loadConfig>;
  agentId: string;
}): AgentRuntimeBackedAcpConfig | null {
  const ownerAgentId = normalizeAgentId(params.agentId);
  const ownerAgent =
    params.cfg.agents?.list?.find((entry) => normalizeAgentId(entry?.id) === ownerAgentId) ??
    null;
  if (!ownerAgent || ownerAgent.runtime?.type !== "acp") {
    return null;
  }

  const acpDefaults = ownerAgent.runtime.acp;
  const harnessAgentId = normalizeAgentId(
    acpDefaults?.agent?.trim() ||
      params.cfg.acp?.defaultAgent?.trim() ||
      resolveDefaultAgentId(params.cfg),
  );
  const backendId = acpDefaults?.backend?.trim() || params.cfg.acp?.backend?.trim() || undefined;
  const cwd = acpDefaults?.cwd?.trim() || undefined;
  const mode = acpDefaults?.mode === "oneshot" ? "oneshot" : "persistent";
...
async function ensureRuntimeBackedAcpSession(params: {
  cfg: ReturnType<typeof loadConfig>;
  acpManager: ReturnType<typeof getAcpSessionManager>;
  sessionKey: string;
  runtimeConfig: AgentRuntimeBackedAcpConfig;
}): Promise<void> {
  const existing = params.acpManager.resolveSession({
    cfg: params.cfg,
  ...
  const agentPolicyError = resolveAcpAgentPolicyError(
    params.cfg,
    params.runtimeConfig.harnessAgentId,
  );
  if (agentPolicyError) {
    throw agentPolicyError;
  }

  await params.acpManager.initializeSession({
    cfg: params.cfg,
    sessionKey: params.sessionKey,
    agent: params.runtimeConfig.harnessAgentId,
    mode: params.runtimeConfig.mode,
    cwd: params.runtimeConfig.cwd,
    backendId: params.runtimeConfig.backendId,
  });
}
...
async function prepareAgentCommandExecution(
  ...
  const runtimeBackedAcp = sessionKey
    ? resolveAgentRuntimeBackedAcpConfig({
        cfg,
        agentId: sessionAgentId,
      })
    : null;
  if (sessionKey && runtimeBackedAcp && !isAcpSessionKey(sessionKey)) {
    const existingAcpResolution = acpManager.resolveSession({
      cfg,
      sessionKey,
    });
    if (existingAcpResolution.kind !== "ready") {
      await ensureRuntimeBackedAcpSession({
        cfg,
        acpManager,
        sessionKey,
        runtimeConfig: runtimeBackedAcp,
      });
      ...
      sessionEntryRaw = sessionResolution.sessionEntry;
      sessionStore = sessionResolution.sessionStore;
      storePath = sessionResolution.storePath;
      isNewSession = sessionResolution.isNewSession;
      persistedThinking = sessionResolution.persistedThinking;
      persistedVerbose = sessionResolution.persistedVerbose;
    }
  }
