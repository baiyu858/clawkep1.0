...
import { PRUNED_HISTORY_IMAGE_MARKER } from "../pi-embedded-runner/run/history-image-prune.js";
import {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      for (const imageRawInput of imageInputs) {
        ...
        if (imageRaw === PRUNED_HISTORY_IMAGE_MARKER) {
          return {
            content: [
              {
                type: "text",
                text: "This image came from older session history and is no longer available to the image tool. Re-upload the image in your latest message, then ask the follow-up again.",
              },
            ],
            details: {
              error: "pruned_history_image_reference",
              image: imageRawInput,
            },
          };
        }
